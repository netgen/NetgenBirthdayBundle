...
<script type="text/javascript">
    var TagsStructureMenuParams = {ldelim}{*
        ...
        *}"autoOpen":false,{*
        *}"tag_id_string":"{'Tag ID'|i18n( 'dms/tags' )|wash( javascript )}",{*
        *}"parent_tag_id_string":"{'Parent tag ID'|i18n( 'dms/tags' )|wash( javascript )}",{*
        *}"treemenu_base_url":"{'tags/treemenu'|ezurl( no )}",{*
        *}"not_allowed_string":"{'Dynamic tree not allowed for this siteaccess'|i18n( 'dms/tags' )|wash( javascript )}",{*
        *}"no_tag_string":"{'Tag does not exist'|i18n( 'dms/tags' )|wash( javascript )}",{*
        *}"internal_error_string":"{'Internal error'|i18n( 'dms/tags' )|wash( javascript )}"{*
    *}{rdelim};
</script>
{/run-once}
